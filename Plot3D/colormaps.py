"""
This module contains colormaps for use in the renderer
"""
# Import modules
import numpy as np
...
class Colormap:
    """
    A class for representing and working with colormaps
    """

    def __init__(self, source_array: np.ndarray):
        """
    ...
    def __call__(self, value):
        # Convert to index and look up in internal array
        value = np.array(value, dtype=int)
        value = np.clip(value, 0, 255)
        return self.__cmap[value]

    @property
    def tex(self) -> int:
        if self.__tex == -1:
            self.__tex = self.toTexture()
        return self.__tex

    @property
    def source(self):
        return self.__cmap.copy()

    def toTexture(self, tex: int = None) -> int:
        """
        Converts colormap into an OpenGL 1D texture

        Arguments:
            tex (int): Existing texture ID to overwrite.  If not provided, a new one is created.

        Returns (int): ID referencing the texture
        """
        # Prepare new texture if not provided
        if tex is None or tex == -1:
        ...
        # Set texture data
        gl.glTexImage1D(gl.GL_TEXTURE_1D, 0, gl.GL_RGB, 255, 0, gl.GL_RGB, gl.GL_FLOAT, self.__cmap)
...
# Declare basic colormaps
viridis = Colormap.fromRGBArray(np.array([[0.267004, 0.004874, 0.329415],
